...
def prepare_coco_captions(filename="Dataset/captions_val2017.json"):
    '''
...
def pad_captions(df):
    ...
    for i, row in dfPadded.iterrows():
        ...
        if(cap_len < max_len):
            ...
            pad_buf = "<PA   print(dfPadded)D> " * pad_len
            pad_buf = pad_buf.strip()
            # dfPadded.set_value(i, 'caption', cap + " " + pad_buf) --changed
            dfPadded.loc[[i],['caption']] = cap + " " + pad_buf
    return dfPadded
...
def get_data(required_files):
    ...
    for fil in required_files:
        ret.append(np.load("Dataset/" + fil + ".npy", allow_pickle=True)) # allow_pickle=True default false
    return ret
